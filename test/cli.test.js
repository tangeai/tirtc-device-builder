...
function createDeviceKit(root, version, sdkVersion = "2.5.0") {
  const required = [
...
test("setup ignores a stale managed Kit reference and selects the pinned Kit", async () => {
  await withTemporaryDirectory(async (directory) => {
    ...
    const expectedKit = join(root, "kits", "esp32s3", "1.1.6");
    ...
    assert.match(result.stdout, /expected 1\.1\.6/);
  });
});
...
test("setup does not accept an explicit older Kit as the pinned Kit", async () => {
  await withTemporaryDirectory(async (directory) => {
    ...
    assert.equal(result.status, 1);
    assert.match(result.stdout, /version 1\.0\.0; expected 1\.1\.6/);
    assert.match(result.stdout, /OVERALL: NEEDS_SETUP/);
  });
});
