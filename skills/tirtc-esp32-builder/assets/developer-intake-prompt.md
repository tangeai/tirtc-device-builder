...
- 选择 SoftAP 时：热点名以 `TiRTC-` 开头、开放认证无需密码，AP 网关和配网页面为 `192.168.6.1` / `http://192.168.6.1`，启用通配 DNS 和 HTTP 重定向进行 captive portal 自动发现，不把 DHCP Option 114 指向本地 HTML 页面，并保留手动打开地址的回退说明
