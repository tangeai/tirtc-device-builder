...
> 本目录是 `tirtc-esp32p4-kit` 的 **ESP32-P4 参考固件**（出处与相对产品应用的改动见 [`../ESP32P4_REFERENCE.md`](../ESP32P4_REFERENCE.md)）。`platform_client`/`runtime_config`/`wifi_manager` 三个共享组件不在本目录内，由兄弟工程 `../device-sim-esp32/components/` 通过 `EXTRA_COMPONENT_DIRS` 提供；由生成器创建的工程会在 `components/` 下物化这些组件。
