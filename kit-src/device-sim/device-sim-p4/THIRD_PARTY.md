...
| 资产 | 当前来源入口 | 保留要求 |
| --- | --- | --- |
| P4 TiRTC SDK | [SDK 组件](components/tirtc_sdk/README.md) | 当前库及配套头文件来自 tirtc-device-builder kit SDK 树的 2.5.0 供应商基线包（`device-sim/sdk/espressif-esp32p4/2.5.0/`，sha256 `9b4e35c7…`），未本地重编译或 strip；保留随包 README 与构建契约，不沿用旧 2.3.0 补丁说明 |
...
| 配网页 Lucide 图标 | [图标许可](../device-sim-esp32/components/wifi_manager/web/ICONS_LICENSE.txt) | ISC / 部分源自 Feather 的 MIT；页面内嵌，无 CDN 依赖；配网页组件由共享组件目录提供 |
