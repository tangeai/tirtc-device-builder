...
| TiRTC SDK | 2.5.0 P4 供应商基线包（sha256 `9b4e35c7…`），库文件与契约位于 kit SDK 树 `device-sim/sdk/espressif-esp32p4/2.5.0/`，接入见 [SDK 组件](../components/tirtc_sdk/README.md) |
| Wi-Fi | C6 + ESP-Hosted 1.4.7 主机补丁组件；从机单独核验，见 [C6 指南](C6_PREPARATION.md) |

SDK 的版本号相同不代表二进制相同。库文件与头文件位于 kit SDK 树，按 [构建契约](../../sdk/espressif-esp32p4/2.5.0/manifest/build-contract.env) 核对（含 `CONFIG_LWIP_MAX_SOCKETS=10`）；Hosted 修改见 [LOCAL_CHANGES](../components/espressif__esp_hosted/LOCAL_CHANGES.md)。
