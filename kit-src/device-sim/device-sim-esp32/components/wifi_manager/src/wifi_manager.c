 ...
 * 提供网络扫描列表、已保存网络复用和手动添加。网页只保存配置并重启，连接状态
 * 仍由同一事件处理路径建立。实时媒体要求关闭 Wi-Fi power save，避免 KCP 音视频
 * 排队和抖动。
 */
...
/* RSSI is informational. Driver queries may block, so only this background
 * worker reads the hardware; UI consumers read a cached snapshot. */
 ...
 * only queues intent and copies metadata; it never blocks on a scan. */
...
static void portal_scan_finish(esp_err_t result)
{
     ...
     * scans. Always release it; do not leave scan memory accumulated. */
...
static esp_err_t nvs_read_record(const char *key, void *value, size_t *size)
{
    nvs_handle_t nvs = 0;
    ...
    err = nvs_get_blob(nvs, key, value, size);
    nvs_close(nvs);
    return err;
}

static esp_err_t nvs_read_string(const char *key, char *value, size_t *size)
{
    nvs_handle_t nvs = 0;
    esp_err_t err = nvs_open(WIFI_NVS_NAMESPACE, NVS_READONLY, &nvs);
    if (err != ESP_OK) {
    ...
    err = nvs_get_str(nvs, key, value, size);
    nvs_close(nvs);
    return err;
}
...
esp_err_t wifi_manager_load_credentials(wifi_manager_credentials_t *credentials)
{
    ...
    esp_err_t err = nvs_read_record(WIFI_NVS_CREDENTIALS, &record, &record_size);
    if (err == ESP_OK) {
        ...
        err = nvs_read_string(WIFI_NVS_SSID, credentials->ssid, &ssid_size);
        if (err == ESP_OK) {
            err = nvs_read_string(WIFI_NVS_PASSWORD, credentials->password, &password_size);
        }
    }
    if (err == ESP_OK &&
...
static esp_err_t portal_networks_get(httpd_req_t *request)
{
    if (!portal_socket_allowed(httpd_req_to_sockfd(request)))
        return httpd_resp_send_err(request, HTTPD_403_FORBIDDEN, "provisioning closed");
    /* HTTPD already uses a PSRAM stack. Never execute an on-demand radio query
     * here: a stalled scan must not hold up the page or its password submission. */
...
static esp_err_t stop_provisioning(void)
{
    ...
    esp_err_t err = wifi_captive_dns_stop();
    if (s_http_server != NULL) {
...
static esp_err_t configure_provisioning_netif(esp_netif_t *ap_netif)
{
    ...
    /* 门户自应答 DNS；同时把 AP 自身作为通告 DNS，避免客户端解析超时。 */
    esp_netif_dns_info_t ap_dns = {0};
    ap_dns.ip.type = ESP_IPADDR_TYPE_V4;
    ap_dns.ip.u_addr.ip4.addr = ip_info.ip.addr;
    err = esp_netif_set_dns_info(ap_netif, ESP_NETIF_DNS_MAIN, &ap_dns);
    if (err != ESP_OK) {
        return err;
    }
...
static esp_err_t start_provisioning(void)
{
    ...
    uint8_t mac[6] = {0};
    /* Use the initialized Wi-Fi interface; the MAC belongs to the active radio
     * (S3 native or P4 Hosted C6), so never read eFuse directly. */
    ...
    if (portal_err == ESP_OK) {
        portal_err = wifi_captive_dns_start(s_ap_netif);
    }
    if (portal_err != ESP_OK) {
...
{
    const char *address = CONFIG_WIFI_MANAGER_FALLBACK_DNS_IPV4;
...
esp_err_t wifi_manager_start(void)
{
    ...
    /* 产品配网页明确约定 192.168.6.1，不能依赖 IDF 的默认 AP 地址。 */
    err = configure_provisioning_netif(s_ap_netif);
    if (err != ESP_OK) {
